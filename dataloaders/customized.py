...
def getMask_query(label,  class_id, class_ids,label_sets):
    """
    ...
    label_l = label_sets*5+1 # voc  *5  or coco *20
    label_r = label_sets*5+5
    #(label == 0) |
    label_reduced = torch.where(((label >= label_l) & (label <= label_r)) ,
                           torch.ones_like(label)*255 , label)

    return {'label_ori':label_reduced,
            'fg_mask': fg_mask,
            'bg_mask': bg_mask}

def getMask(label, scribble, class_id, class_ids,label_sets):
    """
    Generate FG/BG mask from the segmentation mask
    ...
    label_reduced = torch.where(((label >= label_l) & (label <= label_r)) ,
                           torch.ones_like(label)*255 , label)
    #label_reduced = torch.where( ((label >= label_l) & (label <= label_r)) ,
    #                       torch.zeros_like(label) , label)
    #label_reduced = torch.where( ((label > label_r) & (label != 255)),
    #                       label-5 , label)
    #label_reduced = label
...
def fewShot(paired_sample, n_ways, n_shots, cnt_query,label_sets, coco=False):
    """
    ...
    support_mask = [[getMask(support_labels[way][shot], support_scribbles[way][shot],
                             class_ids[way], class_ids,label_sets)
    ...
    query_masks = [getMask_query(query_labels[way], class_ids[way], class_ids,label_sets)
...
def voc_fewshot(base_dir, split, transforms, to_tensor, labels,label_sets, n_ways, n_shots, max_iters,
                n_queries=1):
    """
    Args:
        base_dir:
    ...
    paired_data = PairedDataset(subsets, n_elements=n_elements, max_iters=max_iters, same=False,
                                pair_based_transforms=[
                                    (fewShot, {'n_ways': n_ways, 'n_shots': n_shots,
                                               'cnt_query': cnt_query,'label_sets':label_sets})])
    return paired_data
