...
class Encoder(nn.Module):
    ...
    def forward(self, sup_imgs, qry_imgs, sup_mask):
        ...
        sup_pred_mc = self.classifier(sup_feat)
        ...
        qry_pred_mc = self.classifier(qry_feat)
        n,c,h,w = qry_feat.size()
        prototype = prototype.view(n,c,1,1).expand(-1,-1,h,w)   #1*c*h*w
        feat_in = torch.cat([qry_feat,prototype], dim=1)
        feat_in = self.feat_rd(feat_in)
        feat_in = self.pyramid_m(feat_in)
        pred = self.classifier_2(feat_in)

        #code for robust prototype learning
        feat_in_sup = torch.cat([sup_feat,prototype], dim=1)
        feat_in_sup = self.feat_rd(feat_in_sup)
        feat_in = self.pyramid_m(feat_in)
        pred_sup = self.classifier_2(feat_in_sup)


        #x = self.classifier(x)
        return pred, sup_pred_mc, qry_pred_mc, pred_sup
